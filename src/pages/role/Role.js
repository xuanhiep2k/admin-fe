...
//import Actions
import { getAllRoles as listAllRoles } from "../../redux/actions/roleAction";

function Role() {
  const { userCurrent, isAuth } = useSelector((state) => state.getAuth);
  const dispatch = useDispatch();
  const { loading, error, roles, totalElements, totalPages } = useSelector((state) => (state.getRoles));
  ...
  return (
    <div className="searchrole">
      ...
      <div className="d-flex justify-content-between">
        ...
        <div className="btn-addrole">
          {isAuth && isAuth.length && isAuth.includes("role_create") &&
          <a href="/#" className="btn btn-brand btn-elevate"
      ...
      <table className="table table-bordered table-hover">
        ...
        {roles.length && !loading ?
          (roles.map((role, index) => (
            <tr style={{ "verticalAlign": "middle" }} className="text-center" key={role.code}>
              ...
              <td>
                ...
                < div className="list-user-action">
                  {((isAuth && isAuth.length && isAuth.includes("role_update")) || (userCurrent && userCurrent.roles.includes("ADMIN"))) &&
                  <a href="/#" className="mb-1 mr-1 text-bg-primary" title="Cập nhật quyền"
                     onClick={(e) => handleShowForm(e, role, "update")}>
                    <i className="bi bi-pencil-fill" />
                  </a>}
                  {isAuth && isAuth.length && isAuth.includes("role_lock") && role.code !== "ADMIN" &&
                ...
                <div className="align-items-center justify-content-center list-user-action">
                  {isAuth && isAuth.length && isAuth.includes("role_unlock") &&
                  <a href="/#" className="mb-1 mr-1 text-bg-warning"
                     title="Mở khoá quyền"
                     onClick={(e) => handleShowModal(e, role, "unlock")}>
                    <i className="bi bi-unlock" />
                  </a>}
                  {isAuth && isAuth.length && isAuth.includes("role_delete") &&
        ...
        {roles.length === 0 && !loading ? (<tr className="no-search">

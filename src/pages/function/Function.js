...
import { getALlFunctions } from "../../redux/actions/functionAction";

function Function() {
  const dispatch = useDispatch();
  const { loading, error, functions, totalElements, totalPages } = useSelector((state) => (state.getFunctions));
    ...
    if (dataRow.data.status === "ACTIVE") {
      return (
        <div className="list-user-action">
          {isAuth && isAuth.length && isAuth.includes("function_create") &&
  ...
  return (
    <div className="searchfunc">
      ...
      {!loading && functions.length ? (
        <BootstrapTreeTable columns={treeColumns} tableData={functions} control={treeControls} />
      ) : (
        <>
